...
def detect_request_format(data: Dict[str, Any]) -> str:
    ...
    log.debug(
        f"Unable to definitively detect format, defaulting to OpenAI. Keys present: {list(data.keys())}"
    )
    return "openai"
...
def gemini_request_to_openai(gemini_request: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for content in contents:
        ...
        if len(parts) == 1 and "text" in parts[0]:
            # Simple text message
            openai_request["messages"].append({"role": role, "content": parts[0]["text"]})
        elif len(parts) > 0:
            ...
            if content_parts:
                ...
                if len(content_parts) == 1 and content_parts[0]["type"] == "text":
                    ...
                    openai_request["messages"].append({"role": role, "content": content_parts})
